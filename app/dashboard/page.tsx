...
import MarketIntelligenceList from '@/components/dashboard/MarketIntelligenceList'

export default function DashboardPage() {
  const router = useRouter()
  const { t, isRTL } = useLanguage()
  const { isAuthenticated, isLoading, user, isAdmin } = useAuth()
  const [activeTab, setActiveTab] = useState<'users' | 'requests' | 'marketAnalysis' | 'vipPreviews' | 'marketIntelligence'>('users')
  ...
  const tabs = [
    ...
    { id: 'marketIntelligence' as const, label: t('dashboard.tabs.marketIntelligence'), icon: (
      <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <main className="min-h-screen bg-primary-dark">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div className="overflow-hidden">
          ...
          {activeTab === 'marketIntelligence' && <MarketIntelligenceList />}
        </div>
      </div>
      <Footer />
    </main>
